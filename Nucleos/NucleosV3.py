...
def leer_archivo():
    ...
    global ruta_lote
    global ruta_receta
    global ruta_receta_macro
    global ruta_registro
    global recetas
    global  recetas_macro
    ...
    if "archivo_rutas.txt" in os.listdir(ruta_txt):
        ...
        archivo_rutas = csv.reader(open(ruta_txt + "/archivo_rutas.txt", "r"))
...
def formula_seleccionada(event, sl):
    ...
    try:
        df = pd.read_excel(io=ruta_lote, sheet_name="REGISTRO", header=9)
        df_lote = df["LOTE"].tolist()
        df_mp = df["MATERIA PRIMA"].tolist()
        df_deposito = df["DEPÓSITO"].tolist()
        df_vto = df["FECHA VTO"]
        j = 0
        for i in (df_lote):
    ...
    if (event == "nucleos"):
        ...
        df_receta = pd.read_excel(io=(ruta_receta + "/" + receta_seleccionada), sheet_name="Hoja1", header=0)
    ...
    if (event == "macro"):
        ...
        df_receta = pd.read_excel(io=(ruta_receta_macro + "/" + receta_seleccionada_macro), sheet_name="Hoja1", header=0)
        ...
        if (inicio == True):
            ...
            if MP_seleccionada != mp_selec.get():
                ...
                for i in range(0, n_fila):
                    if MP_seleccionada == hoja_registro.range(n_fila - i, 4).value:
                        if hoja_registro.range(n_fila - i, 7).value != "Eliminado":
                            combobox_lote.delete("0", "end")
                            n_debatch.delete("0", "end")
                            n_debatch.insert(0, int(hoja_registro.range(n_fila - i, 1).value) + 1)
                            if "." in str(hoja_registro.range(n_fila - i, 6).value):
                                combobox_lote.set(str(hoja_registro.range(n_fila - i, 6).value)[
                                                  0:len(str(hoja_registro.range(n_fila - i, 6).value)) - 2])
                            else:
                                combobox_lote.set(str(hoja_registro.range(n_fila - i, 6).value))
                            break
                    else:
    ...
    if(w == "macro"):
        if (inicio_macro == True):
            ...
            if MP_seleccionada_macro != mp_selec_macro.get():
                ...
                for i in range(0, n_fila):
                    if MP_seleccionada_macro == hoja_registro_macro.range(n_fila - i, 4).value:
                        if hoja_registro_macro.range(n_fila - i, 7).value != "Eliminado":
                            combobox_lote_macro.delete("0", "end")
                            if "." in str(hoja_registro_macro.range(n_fila - i, 6).value):
                                combobox_lote_macro.set(str(hoja_registro_macro.range(n_fila - i, 6).value)[0:len(str(hoja_registro_macro.range(n_fila - i, 6).value))-2])
                            else:
                                combobox_lote_macro.set(str(hoja_registro_macro.range(n_fila - i, 6).value))
                            break
        else:
             messagebox.showinfo(message="Debe Iniciar el Proceso", title="Error")
...
def ordenar(col):
    if col == "batch":
        ...
        elem_ordenado = sorted(elem, key=lambda x: x[1])
...
def iniciar(sl):
    global libro_registro
    global libro_registro_macro
    global inicio
    global inicio_macro
    global item
    if (sl == "nucleos"):
        if len(MP) != 0:
            ...
            combobox_lote.delete(0, "end")
            cantidad_pesar.delete(0, "end")
            global hoja_registro
            global hoja_registro_macro
            global n_fila
            if (str(receta_seleccionada).lower() + str(time.strftime("%d-%m-%y")) + ".xls") in str(os.listdir(ruta_registro)).lower():
                libro_registro = xw.Book(ruta_registro + "/" + str(receta_seleccionada).lower()+ str(time.strftime("%d-%m-%y")) + ".xls")
                hoja_registro = xw.sheets[0]
                libro_registro.app.visible = False
                n_fila=2
                while hoja_registro.range(n_fila, 1).value != None:
                    if (hoja_registro.range(n_fila, 7).value!="Eliminado"):
                          hora = (int(hoja_registro.range(n_fila, 3).value * 24))
                          minuto = int((hoja_registro.range(n_fila, 3).value * 24 - int(hoja_registro.range(n_fila, 3).value*24))*60)
                          cuadro2.insert("", tk.END, text=hoja_registro.range(n_fila, 2).value,
                                       values=((str(hora)+":"+str(minuto)+"hs"), int(hoja_registro.range(n_fila, 1).value), hoja_registro.range(n_fila, 4).value, hoja_registro.range(n_fila, 5).value, hoja_registro.range(n_fila, 6).value,
                                               hoja_registro.range(n_fila, 9).value, hoja_registro.range(n_fila, 8).value))
                          n_fila = n_fila + 1
                    else:
                          n_fila = n_fila + 1
            else:
                libro_registro = xw.Book()
                libro_registro.app.visible = False
                hoja_registro = libro_registro.sheets["Hoja1"]
                hoja_registro.range(1, 1).value = "N° de Batch"
                hoja_registro.range(1, 2).value = "Fecha"
                hoja_registro.range(1, 3).value = "Hora"
                hoja_registro.range(1, 4).value = "Materia Prima"
                hoja_registro.range(1, 5).value = "Cantidad"
                hoja_registro.range(1, 6).value = "Lote"
                hoja_registro.range(1, 9).value = "Vencimiento"
                n_fila = 2
    ...
    if (sl == "macro"):
        if len(MP_macro) != 0:
            ...
            if (str(receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".xls") in str(os.listdir(ruta_registro)).lower():
                libro_registro_macro = xw.Book(
                    ruta_registro + "/" + str(receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".xls")
                hoja_registro_macro = xw.sheets[0]
                libro_registro_macro.app.visible = False
                n_fila_macro = 2

                while hoja_registro_macro.range(n_fila_macro, 2).value != None:
                    if (hoja_registro_macro.range(n_fila_macro, 7).value != "Eliminado"):
                        hora = (int(hoja_registro_macro.range(n_fila_macro, 3).value * 24))
                        minuto = int((hoja_registro_macro.range(n_fila_macro, 3).value * 24 - int(
                            hoja_registro_macro.range(n_fila_macro, 3).value * 24)) * 60)
                        cuadro_macro2.insert("", tk.END, text=hoja_registro_macro.range(n_fila_macro, 2).value,
                                       values=(
                                       (str(hora)+":"+str(minuto)+"hs"),int(hoja_registro_macro.range(n_fila_macro, 1).value),
                                       hoja_registro_macro.range(n_fila_macro, 4).value, hoja_registro_macro.range(n_fila_macro, 5).value,
                                       hoja_registro_macro.range(n_fila_macro, 6).value,
                                       hoja_registro_macro.range(n_fila_macro, 9).value, hoja_registro_macro.range(n_fila_macro, 8).value))
                        n_fila_macro = n_fila_macro + 1
                    else:
                        n_fila_macro = n_fila_macro + 1
            else:
                libro_registro_macro = xw.Book()
                libro_registro_macro.app.visible = False
                hoja_registro_macro = libro_registro_macro.sheets["Hoja1"]
                hoja_registro_macro.range(1, 1).value = "Item"
                hoja_registro_macro.range(1, 2).value = "Fecha"
                hoja_registro_macro.range(1, 3).value = "Hora"
                hoja_registro_macro.range(1, 4).value = "Materia Prima"
                hoja_registro_macro.range(1, 5).value = "Cantidad"
                hoja_registro_macro.range(1, 6).value = "Lote"
                hoja_registro_macro.range(1, 9).value = "Vencimiento"
                n_fila_macro = 2
...
def sin_balanza(sec):
    ...
    if(sec == "nucleos"):
        ...
        hoja_registro.range(n_fila, 1).value = n_batch
        hoja_registro.range(n_fila, 2).value = time.strftime("%d/%m/%y")
        hoja_registro.range(n_fila, 3).value = time.strftime("%H:%M:%S")
        hoja_registro.range(n_fila, 4).value = MP_seleccionada
        hoja_registro.range(n_fila, 5).value = (cantidad)
        hoja_registro.range(n_fila, 6).value = lote
        hoja_registro.range(n_fila, 8).value = Deposito[MP.index(MP_seleccionada)]
        hoja_registro.range(n_fila, 9).value = str(lista_vto[lista_lote.index(lote)])
        hoja_registro.range(n_fila, 10).value = str(responsable.get())
        n_fila = n_fila + 1
        n_debatch.delete("0", "end")
        n_debatch.insert(0, int(n_batch) + 1)
        libro_registro.save(
            ruta_registro + "/" + str(receta_seleccionada).lower() + str(time.strftime("%d-%m-%y"))  + ".xls")
    if (sec == "macro"):
        ...
        hoja_registro_macro.range(n_fila_macro, 1).value = item
        hoja_registro_macro.range(n_fila_macro, 2).value = time.strftime("%d/%m/%y")
        hoja_registro_macro.range(n_fila_macro, 3).value = time.strftime("%H:%M:%S")
        hoja_registro_macro.range(n_fila_macro, 4).value = MP_seleccionada_macro
        hoja_registro_macro.range(n_fila_macro, 5).value = (cantidad)
        hoja_registro_macro.range(n_fila_macro, 6).value = lote_macro
        hoja_registro_macro.range(n_fila_macro, 8).value = Deposito_macro[MP_macro.index(MP_seleccionada_macro)]
        hoja_registro_macro.range(n_fila_macro, 9).value = str(lista_vto_macro[lista_lote_macro.index(lote_macro)])
        hoja_registro_macro.range(n_fila_macro, 10).value = str(responsable_macro.get())
        n_fila_macro = n_fila_macro + 1
        item = item + 1
        libro_registro_macro.save(
            ruta_registro + "/" + str(receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".xls")
    ...
    if peso_balanza != "":
        if len(peso_balanza)>8:
            if "NET" in peso_balanza:
                ...
                hoja_registro.range(n_fila, 3).value = time.strftime("%H:%M:%S")
                hoja_registro.range(n_fila, 4).value = MP_seleccionada
                hoja_registro.range(n_fila, 5).value = (peso_mostrar)
                hoja_registro.range(n_fila, 6).value = lote
                hoja_registro.range(n_fila, 8).value = Deposito[MP.index(MP_seleccionada)]
                hoja_registro.range(n_fila, 9).value = str(lista_vto[lista_lote.index(lote)])
                hoja_registro.range(n_fila, 10).value = str(responsable.get())
                ...
                n_debatch.delete("0", "end")
                n_debatch.insert(0, int(n_batch) + 1)
                libro_registro.save(ruta_registro + "/" + str(receta_seleccionada).lower() + str(time.strftime("%d-%m-%y")) + ".xls")
            else:
            ...
            hoja_registro.range(n_fila, 3).value = time.strftime("%H:%M:%S")
            hoja_registro.range(n_fila, 4).value = MP_seleccionada
            hoja_registro.range(n_fila, 5).value = (peso_balanza[1:7])
            hoja_registro.range(n_fila, 6).value = lote
            hoja_registro.range(n_fila, 8).value = Deposito[MP.index(MP_seleccionada)]
            hoja_registro.range(n_fila, 9).value = str(lista_vto[lista_lote.index(lote)])
            hoja_registro.range(n_fila, 10).value = str(responsable.get())
    ...
    if (sector == "nucleos"):
        ...
        try:
            ...
            try:
                ...
                messagebox.showinfo(message="Error en el formato del Archivo de lotes", title="Error en Fecha de Vencimiento")
...
def eliminar(sect):
    if(sect == "nucleos"):
        ...
        if (str(elemento_seleccionado)) == "()":
            ...
            n = 2
            while hoja_registro.range(n, 1).value != None:
                if int(elemento_batch) == int(hoja_registro.range(n, 1).value):
                    if elemento_mp[2] == (hoja_registro.range(n, 4).value):
                        if hoja_registro.range(n, 7).value != "Eliminado":
                            hoja_registro.range(n, 7).value = "Eliminado"
                            cuadro2.delete((elemento_seleccionado))
                            libro_registro.save(
                                ruta_registro + "/" + receta_seleccionada + str(time.strftime("%d-%m-%y")) + ".xls")
                n = n + 1
    if(sect == "macro"):
        ...
        if (str(elemento_seleccionado)) == "()":
            ...
            elemento_batch = elemento_mp[1]
            n = 2
            while hoja_registro_macro.range(n, 1).value != None:
                if int(elemento_batch) == int(hoja_registro_macro.range(n, 1).value):
                    if elemento_mp[2] == hoja_registro_macro.range(n, 4).value:
                        if hoja_registro_macro.range(n, 7).value != "Eliminado":
                            hoja_registro_macro.range(n, 7).value = "Eliminado"
                            cuadro_macro2.delete((elemento_seleccionado))
                            libro_registro_macro.save(
                                ruta_registro + "/" + receta_seleccionada_macro + str(time.strftime("%d-%m-%y")) + ".xls")
                n = n + 1

def cerrar():
    try:
        if inicio == True:
            libro_registro.close()
           # libro_registro.app.Quit()

        if inicio_macro == True:
            libro_registro_macro.close()
            #libro_registro_macro.app.Quit()
        ventana.destroy()
    except:
        ventana.destroy
...
#boton_comprobar = ttk.Button(frame3, text="Verificar Batch", command=verificar)
#boton_comprobar.place(relx=0.905, rely=0.5)
