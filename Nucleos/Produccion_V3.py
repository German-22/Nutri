    ...
    try:
                        ...
                        if parcial == False:    
                            ...
                            if  sector_for =="Macro_Comasa":
                                ...
                                conexion.execute("""insert into simulacion (codprod,formula, mp,deposito, lote, cantidad, vto, ndebatch)
                                    VALUES(?,?,?,?,?,?,?,?);""",(entrada_cod_produccion.get(), combobox.get(), i,dic_deposito[i][m],dic_lote[i][m],round(cant,3),dic_vto[i][m],n))
                                conexion.commit()                         
                                parcial = True
                                m = m + 1                                
                        else:
                            ...
                            if res < 0:                                
                                ...
                                if  sector_for =="Macro_Comasa":
                                    ...
                                    conexion.execute("""insert into simulacion (codprod,formula, mp, deposito, lote, cantidad, vto, ndebatch)
                                    VALUES(?,?,?,?,?,?,?,?);""",(entrada_cod_produccion.get(), combobox.get(), i,dic_deposito[i][m],dic_lote[i][m],round(-(res-cant),3),dic_vto[i][m] ,n))
                                    conexion.commit() 
                                    cant = res
                                    if cant == 0:
                                        m = m + 1
                ...
                if len (dic_stock[i])>m:
                    ...
                    if parcial == True:
                        if  sector_for =="Macro_Comasa":
                            ...
                            conexion.execute("""insert into simulacion (codprod,formula, mp, deposito,lote, cantidad, vto, ndebatch)
                                    VALUES(?,?,?,?,?,?,?,?);""",(entrada_cod_produccion.get(), combobox.get(), i,dic_deposito[i][m],dic_lote[i][m],round(-res,3),dic_vto[i][m] ,n))
                            conexion.commit() 
                    else:
                        n = n + 1
                        if n <= ndebatch:
                            if  sector_for =="Macro_Comasa":
                                ...
                                conexion.execute("""insert into simulacion (codprod,formula, mp, deposito, lote, cantidad, vto, ndebatch)
                                        VALUES(?,?,?,?,?,?,?,?);""",(entrada_cod_produccion.get(), combobox.get(), i,dic_deposito[i][m],dic_lote[i][m],round(cantidad[k],3),dic_vto[i][m] ,n))
                                conexion.commit() 
                                cant = cant - cantidad[k]
            k = k + 1              
        ...
        if  sector_for =="Macro_Comasa":
            ...
            #    for e in dic_calculo:  
                    #a = conexion.execute("""SELECT cantidad FROM simulacion WHERE codprod = ? and mp = ? and lote = ? and estado == "programado";""",(entrada_cod_produccion.get(),dic_calculo[e][2],dic_calculo[e][5]))  
                    #b = a.fetchall()
                    #if b != []:
                    #    conexion.execute("""UPDATE simulacion SET cantidad = ? WHERE codprod = ? and mp = ? and lote = ? and estado == "programado";""",(round(b[0][0]+dic_calculo[e][1],3),entrada_cod_produccion.get(),dic_calculo[e][2],dic_calculo[e][5]))
                    #    conexion.commit()
                    #else:
                   # conexion.execute("""insert into simulacion (codprod,formula, mp, deposito, lote, cantidad, vto, ndebatch)
...
def agrear_mp():
    ...
    conexion=sqlite3.connect(entrada_ruta_bd.get())     
    a = conexion.execute("""SELECT vto from stock WHERE mp = ? and deposito = ? and lote = ?;""", (mp,deposito,lote))
    b = a.fetchall()                      
...
boton_calcular = ttk.Button(pestaña_prod, text="Calcular", command = partial(calcular,""))
boton_calcular.place(relx=0.27, rely=0.01)
boton_nuevo = ttk.Button(pestaña_prod, text="Nuevo", command = partial (nuevo,"nucleos"))
