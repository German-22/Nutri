...
def leer_base():
    ...
    try:
        ...
        combobox_carga['values'] = opciones_formula  
        a = conexion.execute("""SELECT nombre FROM formulas;""")  
        opciones_sim =  a.fetchall()                    
        combobox_sim['values'] = opciones_sim
        conexion.close() 
        buscar("nucleos")       
    except:
...
def calcular(x):    
    ...
    for s in cuadro.get_children():
            cuadro.delete(s)    
    if str(combobox_sim.get()) == "" or entrada_b.get() == "":
        messagebox.showinfo(message="COMPLETE LOS CAMPOS", title="Error")
        return   
    ndebatch = float(entrada_b.get())
    ...
    mostrar_formula(formula,ndebatch)
    return
...
def nuevo(sec):     
    ...
    if  sec == "nucleos":        
        ...
        if lote == "" or ndebatch == "" or formula == "":
            messagebox.showinfo(message="Complete los Campos", title="Error")
            return
        conexion=sqlite3.connect(entrada_ruta_bd.get())   
        ...
        a = conexion.execute("""SELECT nombre, sector, codfor FROM formulas WHERE codfor = ?;""",(b[0][0],))
        b = a.fetchall()
        
        for i in b:                   
            conexion.execute("""insert into producciones (codprod,formula,sector,ndebatch,estado)
                VALUES(?,?,?,?,?);""",(str(lote), i[0], i[1], entrada_ndebatch.get(),"programado"))
        conexion.commit() 
    ...
    conexion.execute("""insert into registro_cambios (fecha,hora,responsable, producto, ndeb, accion, programa, codprod)
                    VALUES(?,?,?,?,?,?,?,?);""",(fecha,hora,respon,formula, ndebatch, "Programar", "Produccion",lote))
    conexion.commit()
    conexion.close()
...
def mostrar_formula(formula,batch):
...
frame_sim = ttk.Frame(pestaña_prod, borderwidth=10, relief="solid")
frame_sim.place(relx=0.6, rely=0.1, relheight=0.8, relwidth=0.4)
combobox_sim = ttk.Combobox(frame_sim, width=30,textvariable=combo_var2)
combobox_sim.place(relx=0.2, rely=0.01,width=300)
combobox_sim.bind('<Return>', partial(filtrar_opciones,combobox_sim,"sim"))
cuadro2 = ttk.Treeview(frame_sim, columns=("Cantidad"))
...
cuadro2.place(relx=0, rely=0.2, relwidth=1, relheight=0.7)
barra2.place(relx=0.95, rely=0.08, relheight=0.9)


boton_calcular = ttk.Button(frame_sim, text="Simular", command = partial(calcular,""))
boton_calcular.place(relx=0, rely=0,relheight = 0.08)
...
#boton_mostrar = ttk.Button(frame_sim, text="Mostrar Formula", command = mostrar_formula)
#boton_mostrar.place(relx=0.55, rely=0.5,relheight = 0.08)
boton_exp = ttk.Button(frame_sim, text="Exportar", command = exportar)
boton_exp.place(relx=0, rely=0.1,relheight = 0.08)
label_ndebatch = ttk.Label(frame_sim, text="N° de Batch")
label_ndebatch.place(relx=0.2, rely=0.12)
entrada_b = ttk.Entry(frame_sim, width=10)
entrada_b.place(relx=0.35, rely=0.12)
