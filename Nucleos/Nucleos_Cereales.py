...
def selecionar_ruta(ruta):
    if ruta == "lote":
        ruta_lote = filedialog.askopenfilename(initialdir="/", title="Seleccionar el Archivo de Ingreso de MP",
                                               filetypes=(("all files", "*.xlsx*"), ("all files", "*.*")))
...
def formula_seleccionada(event, sl):
    ...
    global receta_seleccionada_jarabe
    global df
    global df_lote
    global df_mp
    global df_deposito
    ...
    global Deposito
    global MP_jarabe
    global cantidades_jarabe
    global Deposito_jarabe
    
    try:         
        df = pd.read_excel(io=ruta_lote, sheet_name="REGISTRO", header=5)
    ...
    if (event == "finos"):
        ...
        for mp in (MP):
            ...
            sector[j] = str(sector[j])
            Deposito[j] = str(Deposito[j].lower())
            cuadro.insert("", j, text=mp, values=(cantidades[j], str(sector[j]), Deposito[j]))
    ...
    if (event == "jarabe"):
        ...
        Deposito_jarabe = []
        sector_jarabe = []
        receta_seleccionada_jarabe = combobox_macro.get()
        df_receta = read_excel(io=(ruta_receta_macro + "/" + receta_seleccionada_jarabe), sheet_name="Hoja1", header=0)
        MP_jarabe = df_receta["Materia Prima"].tolist()
        cantidades_jarabe = df_receta["Cantidad"].tolist()
        sector_jarabe = df_receta["Bulto"].tolist()
        Deposito_jarabe = df_receta["Deposito"].tolist()
        peso_batch = 0
        j =0
        for s in cuadro_jarabe.get_children():
            cuadro_jarabe.delete(s)

        for mp in (MP_jarabe):
            cantidades_jarabe[j] = float(cantidades_jarabe[j])
            Deposito_jarabe[j] = str(Deposito_jarabe[j].lower())
            cuadro_jarabe.insert("", j, text=mp, values=(cantidades_jarabe[j], str(sector_jarabe[j]), Deposito_jarabe[j]))
            peso_batch = peso_batch + float(cantidades_jarabe[j])
            sector_jarabe[j] = str(sector_jarabe[j])
            j = j + 1
...
def mp_seleccionada(w,e):
    global MP_seleccionada_jarabe, MP_seleccionada, lista_lote, lista_vto, lista_lote_jarabe, lista_vto_jarabe, sector_selec, sector_selec_jarabe
    
    if(w == "finos"):
        if (inicio == True):
            MP_seleccionada = cuadro.item(cuadro.selection())["text"]
            sector_selec = cuadro.item(cuadro.selection())["values"][1]
    ...
    if(w == "jarabe"):
        if (inicio_macro == True):
            n_debatch_jarabe.delete("0", "end")
            n_debatch_jarabe.insert(0, 1)
            MP_seleccionada_jarabe = cuadro_jarabe.item(cuadro_jarabe.selection())["text"]
            sector_selec_jarabe = cuadro_jarabe.item(cuadro_jarabe.selection())["values"][1]
            if MP_seleccionada_jarabe != mp_selec_jarabe.get():
                lista_vto_jarabe.clear()
                lista_lote_jarabe.clear()
                i = 0
                for o in df_mp:
                    if str(o).lower() == str(MP_seleccionada_jarabe).lower():
                        if (df_deposito[i] == cuadro_jarabe.item(cuadro_jarabe.selection())["values"][2]):
                            lista_lote_jarabe.append(df_lote[i])
                            lista_vto_jarabe.append(df_vto[i])
                    i = i + 1
                mp_selec_jarabe.delete(0, "end")
                combobox_lote_macro.delete(0, "end")
                combobox_lote_macro.set("")
                mp_selec_jarabe.insert(0,MP_seleccionada_jarabe)
                combobox_lote_macro["values"]=lista_lote_jarabe
                cantidad_pesar_macro.delete("0", "end")
                cantidad_pesar_macro.insert(0, float(cantidades_jarabe[MP_jarabe.index(MP_seleccionada_jarabe)]))
...
def iniciar(sl):
    ...
    if (sl == "macro"):
        if len(MP_jarabe) != 0:
            ...
            n_debatch_jarabe.delete(0, "end")
            try:
                if (str(receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt") in str(
                        os.listdir(ruta_registro)).lower():
                    registro_macro = open(str(ruta_registro) + "/" + str(
                        (receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt"), "r")
...
def sin_balanza(sec):
    ...
    if (sec == "jarabe"):
        ...
        cuadro_macro2.insert("", tk.END, text=time.strftime("%d/%m/%y"),
                       values=(time.strftime("%H:%M:%S"),int(n_batch_jarabe),MP_seleccionada_jarabe, cantidad, lote_macro,
                               str(lista_vto_jarabe[lista_lote_jarabe.index(lote_macro)]), Deposito_jarabe[MP_jarabe.index(MP_seleccionada_jarabe)]))
        registro_macro = open(
            ruta_registro + "/" + str((receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt"),
            "a",newline="")
        registro_macro_csv = writer(registro_macro)
        registro_macro_csv.writerow([n_batch_jarabe, time.strftime("%d/%m/%y"), time.strftime("%H:%M:%S"), MP_seleccionada_jarabe, [cantidad], [lote_macro], Deposito_jarabe[MP_jarabe.index(MP_seleccionada_jarabe)],str(lista_vto_jarabe[lista_lote_jarabe.index(lote_macro)]),[str(responsable_macro.get())],[]])
        n_debatch_jarabe.delete("0", "end")
        n_debatch_jarabe.insert(0, int(n_batch_jarabe) + 1)
...
def registrar_finos(peso_balanza):
    if peso_balanza != "":
...
def registrar_jarabe(peso_balanza):
    if peso_balanza != "":
        if len(peso_balanza)>8:
            if "NET" in peso_balanza:
                peso_balanza = [c for c in peso_balanza if c.strip()]
                ...
                cuadro_macro2.insert("", tk.END, text=time.strftime("%d/%m/%y"),
                               values=(time.strftime("%H:%M:%S"), n_batch_jarabe, MP_seleccionada_jarabe, peso_mostrar, lote,str(lista_vto_jarabe[lista_lote_jarabe.index(lote)]),Deposito_jarabe[MP.index(MP_seleccionada_jarabe)]))
                registro_macro = open(
                    ruta_registro + "/" + str(receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt",
                    "a", newline="")
                registro_macro_csv = writer(registro_macro)
                registro_macro_csv.writerow(
                    [n_batch_jarabe, time.strftime("%d/%m/%y"), time.strftime("%H:%M:%S"), MP_seleccionada_jarabe, [peso_mostrar],
                     [lote_macro], [Deposito_jarabe[MP.index(MP_seleccionada_jarabe)]], [str(lista_vto_jarabe[lista_lote_jarabe.index(lote)])],
                     [str(responsable.get())], []])
            ...
            cuadro_macro2.insert("", tk.END, text=time.strftime("%d/%m/%y"),
                           values=(time.strftime("%H:%M:%S"), n_batch_jarabe, MP_seleccionada_jarabe, peso_balanza[1:7], lote_macro,
                                   str(lista_vto_jarabe[lista_lote_jarabe.index(lote)]), Deposito_jarabe[MP.index(MP_seleccionada_jarabe)]))
            registro_macro = open(
                ruta_registro + "/" + str(receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt",
                "a", newline="")
            registro_macro_csv = writer(registro_macro)
            registro_macro_csv.writerow(
                [n_batch_jarabe, time.strftime("%d/%m/%y"), time.strftime("%H:%M:%S"), MP_seleccionada_jarabe, [peso_balanza[1:7]],
                 [lote_macro], [Deposito_jarabe[MP.index(MP_seleccionada_jarabe)]], [str(lista_vto_jarabe[lista_lote_jarabe.index(lote_macro)])],
                 [str(responsable_macro.get())], []])
            registro_macro.close()
            n_fila = n_fila + 1
            n_debatch_jarabe.delete("0", "end")
            n_debatch_jarabe.insert(0, int(n_batch) + 1)

    else:
        messagebox.showinfo(message="Balanza no Conectada", title="Error en Balanza")

def pesar(sector):
    global n_mp, sector_selec, sector_selec_jarabe
    global n_batch, n_batch_jarabe  
    global lote
    global vto
    global lote_macro
        ...
        if lote in lista_lote and venc!= "":
            ...
            if venc > (datetime.strptime(str(datetime.now().date()),"%Y-%m-%d")):
                if responsable.get() != "":
                    ...
                    if sector_selec == "nucleos":                        
                        if float(cantidades[MP.index(MP_seleccionada)]) < 3:                            
                            if act_bal_chica == True:
                                registrar_finos(con_balanza("chica"))
                            else:
                                sin_balanza("finos")
                        else:
                            if act_bal_grande == True:                                
                                registrar_finos(con_balanza("grande"))
                            else:                                
                                sin_balanza("finos")
                    elif sector_selec == "macro":
                        sin_balanza("finos")
                else:
    ...
    if (sector == "jarabe"):
        n_batch_jarabe = n_debatch_jarabe.get()
        lote_macro = combobox_lote_macro.get()
        try:
            venc = datetime.strptime(str(lista_vto_jarabe[lista_lote_jarabe.index(lote_macro)]), "%Y-%m-%d")
        except:
            try:
                    venc = datetime.strptime(str(lista_vto_jarabe[lista_lote_jarabe.index(lote_macro)]), "%d-%m-%Y")
            except:
                messagebox.showinfo(message="Error en el formato del Archivo de lotes",
                                    title="Error en Fecha de Vencimiento")
        if lote_macro in lista_lote_jarabe and venc!="":
            if venc > (datetime.strptime(str(datetime.now().date()),"%Y-%m-%d")):
                    if responsable_macro.get() != "":
                        if sector_selec_jarabe == "nucleos":
                            if float(cantidades_jarabe[MP_jarabe.index(MP_seleccionada_jarabe)]) < 3:
                                if act_bal_chica == True:
                                    registrar_jarabe(con_balanza("chica"))
                                else:
                                    sin_balanza("jarabe")
                            else:
                                if act_bal_grande == True:
                                    registrar_jarabe(con_balanza("grande"))
                                else:
                                    sin_balanza("jarabe")
                        elif sector_selec_jarabe == "macro":
                            sin_balanza("jarabe")        
...
def eliminar(sect):
    ...
    if(sect == "macro"):
        ...
        if (str(elemento_seleccionado)) == "()":
            ...
            registro_macro = open(
                ruta_registro + "/" + str(
                    (receta_seleccionada_jarabe).lower() + str(time.strftime("%d-%m-%y")) + ".txt"),
                "w")
...
tab_control.add(pestaña_nucleos, text="Finos")
tab_control.add(pestaña_macro, text="Jarabe")
...
combobox.bind("<<ComboboxSelected>>", partial(formula_seleccionada,"finos"))
cuadro = ttk.Treeview(frame, columns=("Cantidad", "Sector", "Deposito"))
...
combobox_macro.bind("<<ComboboxSelected>>", partial(formula_seleccionada,"jarabe"))
cuadro_jarabe = ttk.Treeview(frame_macro, columns=("Cantidad", "Sector", "Deposito"))
barra_macro = ttk.Scrollbar(cuadro_jarabe)
cuadro_jarabe.column("#0", width=400, anchor="center")
cuadro_jarabe.column("Cantidad", width=100, anchor="center")
cuadro_jarabe.column("Deposito", width=200, anchor="center")
cuadro_jarabe.column("Sector", width=100, anchor="center")
cuadro_jarabe.heading("#0", text="MP")
cuadro_jarabe.heading("Cantidad", text="Cantidad")
cuadro_jarabe.heading("Sector", text="Sector")
cuadro_jarabe.heading("Deposito", text="Deposito")

cuadro_jarabe.config(yscrollcommand=barra.set)
barra_macro.config(command=cuadro_jarabe.yview)
cuadro_jarabe.place(x=60, y=60, relwidth=0.9, relheight=0.6)
cuadro_jarabe.bind("<<TreeviewSelect>>",partial(mp_seleccionada,"jarabe"))
barra_macro.place(relx=0.97, rely=0.17, relheight=0.81)

label_ndebatch_jarabe = ttk.Label(frame_macro2, text="N° de Batch")
label_ndebatch_jarabe.place(relx=0, rely=0.35)
n_debatch_jarabe = ttk.Entry(frame_macro2, width=7, state="disabled", validate="key",
                      validatecommand=((frame_macro2.register(validar_entrada)), "%S"))
n_debatch_jarabe.place(relx=0.07, rely=0.35)
...
combobox_lote_macro.place(relx=0.45, rely=0.35)
...
mp_selec_jarabe = ttk.Entry(frame_macro2, width=20, state="disabled")
label_cantidad_macro.place(relx=0.3, rely=0.35)
cantidad_pesar_macro.place(relx=0.32, rely=0.35)
label_lote_macro.place(relx=0.4, rely=0.35)
mp_selec_jarabe.place(relx=0.19, rely=0.35)
...
#Pestaña macro
cuadro_macro2 = ttk.Treeview(frame_macro3, columns=( "Hora","Batch", "MP", "Cantidad", "Lote","Vencimiento","Deposito"))
...
cuadro_macro2.column("Batch", width=50, anchor="center")
...
cuadro_macro2.heading("Batch", text="Batch")
...
boton_pesar_macro = ttk.Button(frame_macro2, text="Ingresar", command=partial(pesar,"jarabe"), state="disabled")
...
boton_pesar = ttk.Button(frame2, text="Pesar", command=partial(pesar,"finos"), state="disabled")
