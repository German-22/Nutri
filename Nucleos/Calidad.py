...
def buscar(r,e):
    ...
    for i in b:
        cuadro.insert("", tk.END, text=i[0],
                            values=(i[1],i[2],i[3],i[4],i[5],i[6],i[7],i[8],i[9],i[10],i[11],i[12],i[13],i[14],i[15],i[16],i[17],i[18],i[19],i[20],i[22],i[23],i[24],i[25],i[26],i[27],i[28],i[29]))
    conexion.close()
...
def liberar():
    ...
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    conexion.execute("""UPDATE recepcion SET estado = "liberado" WHERE id = ?;""",(id,))
    conexion.commit()
...
def verificar():
    id = cuadro.item(cuadro.selection())["text"]
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    conexion.execute("""UPDATE recepcion SET estado = "verificado" WHERE id = ?;""",(id,))
    conexion.commit()
    conexion.close()     
    buscar("id",id) 
...
def cargar():
    ...
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    conexion.execute("""UPDATE recepcion SET fechaelaboracion=?,etrevision=?,rotulo=?,organolep=?,fisicoqui=?,contfisico=?,contquimico=?,contmicro=?,responsablecalidad=? WHERE id = ?;""",(fecha_elab,revision,rotulo,organo,fis,confisic,qui,micro,responsable,id))
    conexion.commit()
    conexion.close()     
    buscar("id",id) 
...
def exportar():       
    ...
    with open(ruta + "/" + 'reporte.csv', 'w', newline='') as f:       
        ...
        guardar = ["Fecha","MP","Lote","Fecha de Elaboracion","Vto","Cantidad", "Deposito", "N° de Remito", "Marca","Proveedor","Protocolo","Chofer","Transporte","Patente","Habilitacion","Estado del Transporte","Estado de Carga","Observacion","Responsable","ET Revision","Rotulo","Caracteristicas Organolepticas", "Fisicoquimicas","Contaminantes Fisicos","Contaminantes Quimicos","Contaminantes Micro", "Estado"]
...
fecha = ttk.Combobox(pestaña_prod, width=40)
...
cuadro = ttk.Treeview(pestaña_prod, columns=("Fecha","MP","Lote","Fecha de Elaboracion","Vto","Cantidad", "Deposito", "N° de Remito","Presentacion" , "Marca","Proveedor","Protocolo","Chofer","Transporte","Patente","Habilitacion","Estado del Transporte","Estado de Carga","Observacion","Responsable","ET Revision","Rotulo","Caracteristicas Organolepticas", "Fisicoquimicas","Contaminantes Fisicos","Contaminantes Quimicos","Contaminantes Micro", "Estado"))
...
cuadro.column("Fecha de Elaboracion", width=10, anchor="center")
...
cuadro.column("N° de Remito", width=30, anchor="center")
cuadro.column("Presentacion", width=30, anchor="center")
cuadro.column("Marca", width=20, anchor="center")
cuadro.column("Proveedor", width=20, anchor="center")
cuadro.column("Protocolo", width=20, anchor="center")
cuadro.column("Chofer", width=20, anchor="center")
cuadro.column("Transporte", width=30, anchor="center")
cuadro.column("Patente", width=20, anchor="center")
cuadro.column("Habilitacion", width=10, anchor="center")
cuadro.column("Estado del Transporte", width=10, anchor="center")
cuadro.column("Estado de Carga", width=10, anchor="center")
cuadro.column("Observacion", width=40, anchor="center")
cuadro.column("Responsable", width=20, anchor="center")
cuadro.column("Observacion", width=40, anchor="center")

cuadro.column("ET Revision", width=20, anchor="center")
cuadro.column("Rotulo", width=40, anchor="center")
cuadro.column("Caracteristicas Organolepticas", width=20, anchor="center")
cuadro.column("Fisicoquimicas", width=20, anchor="center")
cuadro.column("Contaminantes Fisicos", width=20, anchor="center")
cuadro.column("Contaminantes Quimicos", width=20, anchor="center")
cuadro.column("Contaminantes Micro", width=20, anchor="center")
...
boton_cargar.place(relx=0.7, rely=0.05, relheight=0.07)
boton_liberar = ttk.Button(pestaña_prod, text="Liberar", command=liberar)
boton_liberar.place(relx=0.7, rely=0.15, relheight=0.07)
boton_verificar = ttk.Button(pestaña_prod, text="verificar", command=verificar)
boton_verificar.place(relx=0.7, rely=0.25, relheight=0.07)
boton_exportar = ttk.Button(pestaña_prod, text="Exportar", command= exportar)
boton_exportar.place(relx=0.3, rely=0.15, relheight=0.07)
