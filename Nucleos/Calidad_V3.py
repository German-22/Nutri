...
def retener_lote_pt():
    producto = producto_combo.get()
    lote = lote_combo.get()
    resp = responsable_desp.get()
    if producto =="" or lote == "" or resp == "":
        messagebox.showinfo(message="Complete los Campos", title="Error")
        return
    comen = comentario_desp.get()
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    conexion.execute('''
                        UPDATE despacho 
                        SET estado = "retenido",responsable_calidad = ?,comentario_calidad = ?            
                        WHERE producto = ? and lote = ?
                    ''', (resp,comen,producto, lote))
    conexion.commit()
    conexion.close()
    cargar_datos(lote)
    return
...
def liberar_lote_pt():
    ...
    conexion.execute('''
                        UPDATE despacho 
                        SET estado = "liberado",responsable_calidad = ?,comentario_calidad = ?            
                        WHERE producto = ? and lote = ?
                    ''', (resp,comen,producto, lote))
    conexion.commit()
    conexion.close()
...
pestaña_desp = ttk.Frame(tab_control, borderwidth=10, relief="solid")
pestaña_desp.place(x=0, y=0, relheight=1, relwidth=1)
pestaña_config = ttk.Frame(tab_control, borderwidth=10, relief="solid")
pestaña_config.place(x=0, y=0, relheight=1, relwidth=1)
tab_control.add(pestaña_prod, text="Calidad MP")
tab_control.add(pestaña_desp, text="Calidad PT")
...
btn_retener_lote= Button(pestaña_desp, text="Retener Lote",command=retener_lote_pt)
btn_retener_lote.place(relx=0.25, rely=0.08)
btn_liberar_lote= Button(pestaña_desp, text="Liberar Lote",command=liberar_lote_pt)
btn_liberar_lote.place(relx=0.2, rely=0.08)
btn_retener= Button(pestaña_desp, text="Retener",command=retener_pt)
btn_retener.place(relx=0.65, rely=0.3,relheight=0.07,relwidth=0.08)
btn_liberar= Button(pestaña_desp, text="Liberar",command=liberar_pt)
btn_liberar.place(relx=0.65, rely=0.4,relheight=0.07,relwidth=0.08)
ttk.Label(pestaña_desp, text="Responsable:").place(relx=0.37,rely=0.01)
responsable_desp = ttk.Entry(pestaña_desp, width=30)
responsable_desp.place(relx=0.45,rely=0.01)
ttk.Label(pestaña_desp, text="Comentario:").place(relx=0.37,rely=0.1)
comentario_desp = ttk.Entry(pestaña_desp, width=30)
comentario_desp.place(relx=0.45,rely=0.1)  



contenedor_tree = ttk.Frame(pestaña_desp)
...
tree = ttk.Treeview(
    contenedor_tree,
    columns=("id", "Fecha", "Producto", "Pallet", "Lote", "Vto", "Cajas","Estado", "Comentario","Responsable","Responsable Calidad","Comentario Calidad"),
    show='headings',
