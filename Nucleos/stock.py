...
def buscar():
    for s in cuadro.get_children():
        cuadro.delete(s)
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    a = conexion.execute("""SELECT * FROM stock WHERE mp = ? or lote = ? or deposito = ?;""", (combobox.get(),combobox_lote.get(),combobox_deposito.get()))         
    b = a.fetchall()  
    for i in b:
        cuadro.insert("", tk.END, text=i[0],
                            values=(i[2],i[5],i[3],i[1]))
    conexion.close()
...
def actualizar():
    ...
    combobox.insert(0,mp)
    conexion=sqlite3.connect(entrada_ruta_bd.get())    
    if nuevo_stock != "":        
        nuevo_stock = float(nuevo_stock)
        if nuevo_stock == 0:
            conexion.execute("""UPDATE stock SET stock = ? WHERE mp = ? and lote = ?;""",(nuevo_stock,mp,lote))
            conexion.commit()
            conexion.execute("""UPDATE stock SET estado = "agotado" WHERE mp = ? and lote = ?;""",(mp,lote))
            conexion.commit()
        else:
...
cuadro = ttk.Treeview(pestaña_prod, columns=("Lote","Vto","Stock", "Deposito"))
...
boton_buscar.place(relx=0.4, rely=0.15, relheight=0.07)
boton_exportar = ttk.Button(pestaña_prod, text="Exportar", command= exportar)
boton_exportar.place(relx=0.4, rely=0.3, relheight=0.07)
...
boton_act.place(relx=0.7, rely=0.23,relheight=0.07)
entrada_stock = ttk.Entry(pestaña_prod, width=10)
entrada_stock.place(relx=0.6, rely=0.25)
leer_archivo()
leer_base()
ventana.mainloop()
