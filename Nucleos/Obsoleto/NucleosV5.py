            ...
            if MP_seleccionada != mp_selec.get():
                ...
                try:
                    if (str(receta_seleccionada).lower() + str(time.strftime("%d-%m-%y")) + ".txt") in str(
                        ...
                        leer_reg_nucleos = csv.reader(registro)
                        lista = (list(leer_reg_nucleos))
                        lote = []
                        for f in lista:
                            if len(f) != 0 and f[9] != "Eliminado":
                                if str(f[3])==str([str(MP_seleccionada)]):
                                   n_batch+=1
        ...
        if len(MP_macro) != 0:
            ...
            try:
                if (str(receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".txt") in str(
                    ...
                    leer_reg = csv.reader(registro_macro)
...
def sin_balanza(sec):
    ...
    if(sec == "nucleos"):
        ...
        registro_csv = csv.writer(registro)
    ...
    if (sec == "macro"):
        ...
        registro_macro = open(
            ...
            "a",newline="")
        registro_macro_csv = csv.writer(registro_macro)
...
def eliminar(sect):
    if(sect == "nucleos"):
        ...
        if (str(elemento_seleccionado)) == "()":
            ...
            for f in list(reg):
                if len(f) != 0:
                    if str(f[0]) == str([str(elemento_batch)]):
                        registro_csv = csv.writer(registro)
...
def cerrar():
    if combobox.get()!="":
        if (str(combobox.get()).lower() + str(time.strftime("%d-%m-%y")) + ".txt") in str(os.listdir(ruta_registro)).lower():
            try:
                ...
                hoja_registro.range(1, 7).value = "Deposito"
                hoja_registro.range(1, 8).value = "Vencimiento"
                hoja_registro.range(1, 9).value = "Responsable"
                hoja_registro.range(1, 10).value = "Estado"
                registro = open(str(ruta_registro) + "/" + str((receta_seleccionada).lower() + str(time.strftime("%d-%m-%y")) + ".txt"), "r")
                leer_reg_nucleos = csv.reader(registro)
                n_fila=2
                for f in list(leer_reg_nucleos):
                    if len(f) != 0:
                        hoja_registro.range(n_fila, 1).value = f[0]
                        hoja_registro.range(n_fila, 2).value = f[1]
                        hoja_registro.range(n_fila, 3).value = f[2]
                        hoja_registro.range(n_fila, 4).value = f[3]
                        hoja_registro.range(n_fila, 5).value = f[4]
                        hoja_registro.range(n_fila, 6).value = f[5]
                        hoja_registro.range(n_fila, 7).value = f[6]
                        hoja_registro.range(n_fila, 8).value = f[7]
                        hoja_registro.range(n_fila, 9).value = f[8]
                        hoja_registro.range(n_fila, 10).value = f[9]
                        n_fila = n_fila + 1
                libro_registro.save(ruta_registro + "/" + str(receta_seleccionada).lower() + str(time.strftime("%d-%m-%y")) + ".xls")
                libro_registro.close()

            except:
                messagebox.showinfo(message="Problema con Excel, no se genero el archivo", title="Error")
                libro_registro.close()

    if combobox_macro.get()!="":
        if (str(combobox_macro.get()).lower() + str(time.strftime("%d-%m-%y")) + ".txt") in str(os.listdir(ruta_registro)).lower():
            try:
                libro_registro = xw.Book()
                libro_registro.app.visible = False
                hoja_registro = libro_registro.sheets["Hoja1"]
                hoja_registro.range(1, 1).value = "Item"
                hoja_registro.range(1, 2).value = "Fecha"
                hoja_registro.range(1, 3).value = "Hora"
                hoja_registro.range(1, 4).value = "Materia Prima"
                hoja_registro.range(1, 5).value = "Cantidad"
                ...
                hoja_registro.range(1, 9).value = "Responsable"
                hoja_registro.range(1, 10).value = "Estado"
                registro = open(str(ruta_registro) + "/" + str((receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".txt"), "r")
                leer_reg = csv.reader(registro)
                n_fila=2
                for f in list(leer_reg):
                    if len(f) != 0:
                        ...
                        hoja_registro.range(n_fila, 10).value = f[9]
                        n_fila = n_fila + 1
                libro_registro.save(ruta_registro + "/" + str(receta_seleccionada_macro).lower() + str(time.strftime("%d-%m-%y")) + ".xls")
                libro_registro.close()
