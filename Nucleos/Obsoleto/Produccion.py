...
def calcular(x):
    ndebatch_anterior = 0
    conexion=sqlite3.connect(entrada_ruta_bd.get())
    a = conexion.execute("""SELECT codprod FROM producciones WHERE codprod = ?;""", (entrada_cod_produccion.get(),))
    b = a.fetchall()
    if x == "agregar":
        if b == []:
        ...
        for i in dic_stock:#Recorro las MP en stock
            ...
            while n<ndebatch:                    
                    ...
                    if len(dic_stock[i]) > m:  
                        ...
                        if parcial == False:    
                            ...
                            cuadro.insert("", tk.END, text=entrada_cod_produccion.get(),
                                values=(i,formula,n,cant,dic_deposito[i][m],dic_lote[i][m],dic_vto[i][m]))                       
                            ...
                            if res < 0:
                                ...
                                cuadro.insert("", tk.END, text=entrada_cod_produccion.get(),
                                values=(i,formula,n,-(res-cant),dic_deposito[i][m],dic_lote[i][m],dic_vto[i][m]))
                                conexion.execute("""insert into simulacion (codprod,formula, mp, deposito, lote, cantidad, vto, ndebatch)
                ...
                if len (dic_stock[i])>m:
                    ...
                    if parcial == True:
                        cuadro.insert("", tk.END, text=entrada_cod_produccion.get(),
                            values=(i,formula,n,-res,dic_deposito[i][m],dic_lote[i][m],dic_vto[i][m]))
                        conexion.execute("""insert into simulacion (codprod,formula, mp, deposito,lote, cantidad, vto, ndebatch)
                        ...
                        if n <= ndebatch:
                            cuadro.insert("", tk.END, text=entrada_cod_produccion.get(),
                                values=(i,formula,n,cantidad[k],dic_deposito[i][m],dic_lote[i][m],dic_vto[i][m]))
                            conexion.execute("""insert into simulacion (codprod,formula, mp, deposito, lote, cantidad, vto, ndebatch)
...
boton_eliminar_carga = ttk.Button(pestaña_carga, text="Eliminar", command=partial(eliminar,"carga"))
boton_eliminar_carga.place(relx=0.27, rely=0.13)
